...
def _save_cache(cache_dir: Path, fingerprint: str, embeddings: np.ndarray, chunks: Sequence[ChunkRecord]) -> str:
    ...
    try:
        ...
        payload = {"fingerprint": fingerprint, "chunks": _serialize_chunks(chunks)}
...
def _load_cache(cache_dir: Path, fingerprint: str) -> tuple[np.ndarray, List[ChunkRecord]] | None:
    npz_path, json_path = _get_cache_paths(cache_dir)
...
def build_rag_store(
    ...
    if cached:
        ...
        cache_status = _save_cache(cache_dir, fingerprint, embeddings, chunks)
...
def answer_question(
    ...
    if intent == "ambiguous":
        return "¿Te referís a resultados ganaderos, agrícolas, financieros, reproductivos o de una campaña específica?"
