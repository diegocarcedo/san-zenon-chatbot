...
def main() -> int:
    ...
    if 'st.tabs(["Chat", "Diagnóstico"])' in code or "st.tabs(['Chat', 'Diagnóstico'])" in code:
        print("ERROR: patrón legado exacto st.tabs([\"Chat\", \"Diagnóstico\"]) detectado")
        return 1
    if re.search(r"\bst\.tabs\s*\(", code) and ("Chat" in code and "Diagnóstico" in code):
        ...
        return 1
    if "Ver snippets recuperados" in code:
        print("ERROR: patrón legado 'Ver snippets recuperados' detectado")
        return 1
    if "last_retrieved" in code:
        print("ERROR: patrón legado 'last_retrieved' detectado")
        return 1
